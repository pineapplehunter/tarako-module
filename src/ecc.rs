...
extern "C" {
    ...
    pub fn ecc_digits_from_bytes(inp: *const u8, nbytes: c_uint, out: *mut u64, ndigits: c_uint);
    pub fn vli_mod_inv(
        result: *mut u64,
        input: *const u64,
        modulus: *const u64,
    ...
    pub fn vli_sub(result: *mut u64, left: *const u64, right: *const u64, ndigits: c_uint) -> u64;
...
pub fn generate_private_key() -> Result<[u64; 4]> {
    let mut key = [0u64; 4];
...
pub fn make_public_key(privkey: &[u64; 4]) -> Result<[u64; 8]> {
...
pub fn digits_from_be_bytes(bytes: &[u8; 32]) -> [u64; 4] {
    let mut out = [0u64; 4];
...
pub fn vli_mod_inv_result(input: &[u64; 4], modulus: &[u64; 4]) -> [u64; 4] {
    let mut result = [0u64; 4];
    unsafe { vli_mod_inv(result.as_mut_ptr(), input.as_ptr(), modulus.as_ptr(), DIGITS) };
    result
}

/// Compute modular multiplication: `result = left * right mod modulus`.
pub fn vli_mod_mult(left: &[u64; 4], right: &[u64; 4], modulus: &[u64; 4]) -> [u64; 4] {
    let mut result = [0u64; 4];
    unsafe {
        vli_mod_mult_slow(result.as_mut_ptr(), left.as_ptr(), right.as_ptr(), modulus.as_ptr(), DIGITS)
    };
    result
}
...
pub fn vli_sub_result(left: &[u64; 4], right: &[u64; 4]) -> ([u64; 4], u64) {
    let mut result = *left;
...
pub fn vli_compare(left: &[u64; 4], right: &[u64; 4]) -> core::cmp::Ordering {
    let ret = unsafe { vli_cmp(left.as_ptr(), right.as_ptr(), DIGITS) };
    ret.cmp(&0)
}

/// Check if an LE-limb number is zero.
pub fn is_vli_zero(vli: &[u64; 4]) -> bool {
    unsafe { vli_is_zero(vli.as_ptr(), DIGITS) }
}

/// Record the public key via IMA.
pub fn ima_measure_pubkey(pubkey: &[u8; 65]) {
    unsafe {
