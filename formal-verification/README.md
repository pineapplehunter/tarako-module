...
[`success-trace.svg`](success-trace.svg) is generated by ProVerif's own trace reconstruction. It shows one execution from the fresh client request through the TDX and IMA appraisal services, Tarako signing, the relying verifier's verdict, and `ClientAcceptedIntegrity`. Because the network is public, messages appear as sends to and receives from the Dolev–Yao attacker, even though this witness forwards the honest protocol messages.

ProVerif normally reconstructs traces only for queries it cannot prove. [`generate-success-trace.py`](generate-success-trace.py) therefore creates a temporary model that replaces the security queries with this diagnostic reachability query:
...
A fact-only ProVerif query abbreviates `event(...) ==> false`. The expected result
...
```sh
python3 formal-verification/generate-success-trace.py
```
...
- [`test-negative-models.py`](test-negative-models.py) — positive proof and required-counterexample mutation suite.
- [`generate-success-trace.py`](generate-success-trace.py) — successful-run reachability trace generator.
- [`success-trace.dot`](success-trace.dot) and [`success-trace.svg`](success-trace.svg) — ProVerif-generated end-to-end witness trace.
